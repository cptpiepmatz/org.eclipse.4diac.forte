...
EComResponse CDDSComLayer::openConnection(char *pa_acLayerParameter) {
  ...
  DEVLOG_DEBUG("[DDSLAYER] Topic name is '");
  DEVLOG_DEBUG(this->m_TopicName.c_str());
  DEVLOG_DEBUG("'.\n");

  this->m_TopicType = layerParams.substr(colon + 1);
  DEVLOG_DEBUG("[DDSLAYER] Topic type is '");
  DEVLOG_DEBUG(this->m_TopicType.c_str());
  DEVLOG_DEBUG("'.\n");

  switch (this->m_eCommServiceType) {
    case EComServiceType::e_Subscriber:
      DEVLOG_ERROR("[DDSLAYER] Subscriber are not supported.\n");
      return EComResponse::e_InitInvalidId;
    case EComServiceType::e_Server:
      DEVLOG_ERROR("[DDSLAYER] Server are not supported.\n");
      return EComResponse::e_InitInvalidId;
    case EComServiceType::e_Client:
      DEVLOG_ERROR("[DDSLAYER] Clients are not supported.\n");
      return EComResponse::e_InitInvalidId;
  }

  if (this->m_eCommServiceType == EComServiceType::e_Publisher) {
    if (this->getCommFB()->getNumSD() != 1) {
      DEVLOG_ERROR("[DDSLAYER] Publishers need exactly 1 SD input.\n");
      return EComResponse::e_InitInvalidId;
    }

    if (this->getCommFB()->getSDs()->getDataTypeID() != CIEC_ANY::EDataTypeID::e_STRUCT) {
      DEVLOG_ERROR("[DDSLAYER] Only STRUCT is supported.\n");
      return EComResponse::e_InitInvalidId;
    }
    ...
    if (this->publisher == nullptr) {
      DEVLOG_ERROR("[DDSLAYER] Topic type unknown.\n");
      return EComResponse::e_InitInvalidId;
    }
    if (!this->publisher->validateType(data->getStructTypeNameID())) {
      DEVLOG_ERROR("[DDSLAYER] Data type not correct.\n");
      return EComResponse::e_InitInvalidId;
    }
    if (!this->publisher->init()) {
      DEVLOG_ERROR("[DDSLAYER] Could not initialize publisher.\n");
      return EComResponse::e_InitInvalidId;
    }
  }
