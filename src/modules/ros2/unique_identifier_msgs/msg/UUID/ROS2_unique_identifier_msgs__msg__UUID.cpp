/*************************************************************************
 *** FORTE Library Element
 ***
 *** This file was generated using the 4DIAC FORTE Export Filter V1.0.x NG!
 ***
 *** Name: ROS2_unique_identifier_msgs__msg__UUID
 *** Description: 
 *** Version:
***     1.0: 2023-03-15/Tim Hesse -  - 
 *************************************************************************/

#include "ROS2_unique_identifier_msgs__msg__UUID.h"
#ifdef FORTE_ENABLE_GENERATED_SOURCE_CPP
#include "ROS2_unique_identifier_msgs__msg__UUID_gen.cpp"
#endif

DEFINE_FIRMWARE_DATATYPE(ROS2_unique_identifier_msgs__msg__UUID, g_nStringIdROS2_unique_identifier_msgs__msg__UUID);

CIEC_ROS2_unique_identifier_msgs__msg__UUID::CIEC_ROS2_unique_identifier_msgs__msg__UUID() :
    CIEC_STRUCT(g_nStringIdROS2_unique_identifier_msgs__msg__UUID, 1, scmElementTypes, scmElementNames, e_APPLICATION + e_CONSTRUCTED + 1) {
}

const CStringDictionary::TStringId CIEC_ROS2_unique_identifier_msgs__msg__UUID::scmElementNames[] = {g_nStringIduuid};
const CStringDictionary::TStringId CIEC_ROS2_unique_identifier_msgs__msg__UUID::scmElementTypes[] = {g_nStringIdARRAY, 16, g_nStringIdUSINT};


